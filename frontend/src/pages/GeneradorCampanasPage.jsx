...
import { inputClass, btnPrimary, btnSecondary } from '../styles/appStyles';
...
const operadoresOptions = [
    ...
    { label: 'Comienza con', value: 'comienza_con' }, 
...
function GeneradorCampanasPage() {
    ...
    // Variable de Bloqueo Global (Deshabilita UI durante ejecución)
    const isLocked = loading || executionStatus === 'running';
    ...
    const proveedoresConfig = useMemo(() => ({
        ...
        'DISC': [{ label: 'Neotel', value: 'NEOTEL' }]
    }), []);
    ...
    const fetchPlantillas = async () => { 
        setLoading(true);
        try { const res = await axios.get(`${API_CAMPANAS_URL}/plantillas`, { headers: { Authorization: `Bearer ${token}` } }); setPlantillasGuardadas(res.data); } 
        catch (e) {} 
    ...
    useEffect(() => { 
        if (!selectedCliente || !token) { setEstrategiasDisponibles([]); return; } 
        if (isEditing && estrategiasDisponibles.length > 0) return; 
        const fetch = async () => { try { const res = await axios.get(`${API_VISOR_URL}/strategies/${selectedCliente}`, { headers: { Authorization: `Bearer ${token}` } }); setEstrategiasDisponibles(res.data.map(e => ({ label: e.nombre, value: e.id }))); } catch (e) {} }; 
        fetch(); 
    }, [selectedCliente, token, isEditing]);
    
    useEffect(() => { 
        if (!selectedEstrategia || !token) { 
            setColumnasDisponibles([]); setStrategyData(null); return; 
        } 
        const fetch = async () => { 
            try { 
                const res = await axios.get(`${API_VISOR_URL}/strategies/load/${selectedEstrategia}`, { headers: { Authorization: `Bearer ${token}` } }); 
    ...
    const handleGuardar = async () => {
        ...
        if ((tipoCampana === 'MAIL' || tipoCampana === 'MAIL_INF') && emailStrategy === 'unica' && !selectedEmailCol) { toast.current.show({ severity: 'warn', summary: 'Falta Columna Email', detail: 'Seleccione la columna de correo.' }); return; }
    ...
    const handleCheckAndRun = async (rowData) => { 
        ...
        try { 
            ...
            if (res.data.files && res.data.files.length > 0) { setExistingFilesList(res.data.files); setExistingFilesDialogVisible(true); setLoading(false); } 
        ...
        return (
            <div className="mt-3 p-3 bg-blue-50 border border-blue-100 rounded text-sm text-blue-800">
                ...
                <ul className="list-disc pl-5 space-y-1">
                    ...
                    {tipoCampana === 'DISC' && ( <> <li>Generación de archivo exclusivo de RUTs (Neotel).</li> </> )}
            ...
            <TabView activeIndex={activeIndex} onTabChange={(e) => { setActiveIndex(e.index); if(e.index === 0 && !isEditing) resetForm(); }}>
                <TabPanel header="Mis Plantillas">
                    <div className="mb-4 flex justify-between">
                        <Button label="Refrescar" icon="pi pi-refresh" onClick={fetchPlantillas} size="small" className={btnSecondary} disabled={isLocked} />
                        {isAdmin && <Button label="Nueva Plantilla" icon="pi pi-plus" onClick={() => { resetForm(); setActiveIndex(1); }} size="small" className={btnPrimary} disabled={isLocked} />}
                    </div>
                    <DataTable key={isLocked ? 'locked' : 'unlocked'} value={plantillasGuardadas} stripedRows size="small" emptyMessage="No hay plantillas disponibles.">
                        <Column field="id" header="ID" sortable style={{width:'60px'}}/>
                        <Column field="nombre_plantilla" header="Nombre" sortable />
                        {isAdmin && (<Column field="estado" header="Estado" body={(r) => (<InputSwitch checked={r.estado === 1} onChange={(e) => handleToggleEstado(r, e.value)} disabled={isLocked} />)} sortable style={{width:'100px'}}/>)}
                        ...
                        <Column header="Acciones" body={(rowData) => (
                            <div className="flex gap-2">
                                {rowData.estado !== 0 && (
                                    <Button icon="pi pi-play" className="p-button-rounded p-button-success p-button-text" onClick={() => handleCheckAndRun(rowData)} tooltip="Ejecutar" disabled={isLocked} />
                                )}
                                {isAdmin && rowData.estado !== 0 && (
                                    <Button icon="pi pi-pencil" className="p-button-rounded p-button-info p-button-text" onClick={() => handleEditPlantilla(rowData)} tooltip="Editar" disabled={isLocked} />
                                )}
                            </div>
                        )} />
                    </DataTable>
                    ...
                    <TabPanel header={isEditing ? "Editar Plantilla" : "Nueva Plantilla"}>
                        <div className="flex flex-col gap-6 max-w-5xl mx-auto">
                            <div className="flex flex-col gap-2">
                                ...
                                <InputText value={nombrePlantilla} onChange={(e) => setNombrePlantilla(e.target.value)} className={inputClass} disabled={isLocked} />
                            </div>
                            <div className="p-5 border border-gray-200 rounded-lg bg-white shadow-sm">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div><label className="block mb-1 text-sm font-semibold">Cliente</label><Dropdown value={selectedCliente} options={clientesDisponibles} onChange={(e) => setSelectedCliente(e.value)} className="w-full" filter disabled={isLocked} /></div>
                                    <div><label className="block mb-1 text-sm font-semibold">Estrategia Base</label><Dropdown value={selectedEstrategia} options={estrategiasDisponibles} onChange={(e) => setSelectedEstrategia(e.value)} className="w-full" disabled={!selectedCliente || isLocked} filter /></div>
                                </div>
                                {strategyData && (
                                    <div className="mt-3">
                            ...
                            <div className="p-5 border border-gray-200 rounded-lg bg-white shadow-sm">
                                ...
                                <div className="grid grid-cols-2 gap-6 mb-4">
                                    <div><label className="block mb-2 text-sm font-semibold">Tipo de Campaña</label><Dropdown value={tipoCampana} options={tiposCampanaOptions} onChange={(e) => setTipoCampana(e.value)} placeholder="Seleccione..." className="w-full" disabled={isLocked} /></div>
                                    <div><label className="block mb-2 text-sm font-semibold">Proveedor de Envío</label><Dropdown value={selectedProveedor} options={tipoCampana ? proveedoresConfig[tipoCampana] : []} onChange={(e) => setSelectedProveedor(e.value)} placeholder="Seleccione..." className="w-full" disabled={!tipoCampana || isLocked} /></div>
                                </div>
                                ...
                                {(tipoCampana === 'MAIL' || tipoCampana === 'MAIL_INF') && (
                                    <div className="bg-blue-50 p-3 rounded border border-blue-100 mb-3">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <div className="flex align-items-center"><input type="radio" name="emailStrat" value="jerarquia" checked={emailStrategy === 'jerarquia'} onChange={(e) => setEmailStrategy(e.target.value)} className="w-4 h-4 text-blue-600" disabled={isLocked}/><label className="ml-2 text-sm text-gray-700"><strong>Jerarquía Automática:</strong> Buscar el primer correo válido (Mail1 → Mail2...).</label></div>
                                            <div className="flex align-items-center"><input type="radio" name="emailStrat" value="unica" checked={emailStrategy === 'unica'} onChange={(e) => setEmailStrategy(e.target.value)} className="w-4 h-4 text-blue-600" disabled={isLocked}/><label className="ml-2 text-sm text-gray-700"><strong>Columna Específica:</strong> Usar solo una columna. Si falla, se rechaza.</label></div>
                                            {emailStrategy === 'unica' && (<div className="ml-6 mt-1"><Dropdown value={selectedEmailCol} options={emailColumnsAvailable} onChange={(e) => setSelectedEmailCol(e.value)} placeholder="Seleccione columna..." className="w-full md:w-1/2 p-inputtext-sm" filter disabled={isLocked} /></div>)}
                                        </div>
                                    </div>
                                )}
                                ...
                                {tipoCampana === 'SMS' && (
                                    <div className="bg-blue-50 p-3 rounded border border-blue-100 mb-3">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <div className="flex align-items-center"><input type="radio" name="fonoStrat" value="jerarquia" checked={fonoStrategy === 'jerarquia'} onChange={(e) => setFonoStrategy(e.target.value)} className="w-4 h-4 text-blue-600" disabled={isLocked}/><label className="ml-2 text-sm text-gray-700"><strong>Jerarquía Automática:</strong> Buscar el primer teléfono válido (Fono1 → Fono2...).</label></div>
                                            <div className="flex align-items-center"><input type="radio" name="fonoStrat" value="unica" checked={fonoStrategy === 'unica'} onChange={(e) => setFonoStrategy(e.target.value)} className="w-4 h-4 text-blue-600" disabled={isLocked}/><label className="ml-2 text-sm text-gray-700"><strong>Columna Específica:</strong> Usar solo una columna.</label></div>
                                            {fonoStrategy === 'unica' && (<div className="ml-6 mt-1"><Dropdown value={selectedFonoCol} options={fonoColumnsAvailable} onChange={(e) => setSelectedFonoCol(e.value)} placeholder="Seleccione columna..." className="w-full md:w-1/2 p-inputtext-sm" filter disabled={isLocked} /></div>)}
                                            
                                            <div className="mt-4 p-3 bg-white rounded border border-gray-300">
                                                <label className="block mb-1 text-sm font-bold text-gray-700">Mensaje SMS (Máx 160 chars)</label>
                                                <InputTextarea ref={smsTextAreaRef} value={mensajeSMS} onChange={(e) => setMensajeSMS(e.target.value)} rows={3} maxLength={160} className="w-full font-mono text-sm" placeholder="Hola {nombre}, recuerda pagar tu deuda de {deuda}..." disabled={isLocked} />
                                                <div className="flex justify-between items-center mt-2">
                                                    ...
                                                    <div className="flex gap-2 items-center"><span className="text-xs text-gray-500">Insertar variable:</span><Dropdown value={colToInsertSMS} options={availableColumns} onChange={(e) => insertColumnToSMS(e.value)} placeholder="Columna..." className="p-inputtext-sm w-40" filter disabled={isLocked} /></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="p-5 border border-gray-200 rounded-lg bg-white shadow-sm">
                                ...
                                <MultiSelect value={columnasDivision} options={columnasDisponibles} onChange={(e) => setColumnasDivision(e.value)} className="w-full" display="chip" filter disabled={isLocked} />
                            </div>
                            ...
                            <div className="p-5 border border-gray-200 rounded-lg bg-white shadow-sm">
                                ...
                                <div className="mb-4 flex flex-col gap-2 bg-gray-50 p-3 rounded border">
                                    <div className="flex gap-2 items-center mb-2">
                                        ...
                                        <InputText value={tempSegSufijo} onChange={(e) => setTempSegSufijo(e.target.value)} placeholder="Sufijo (_VIP)" className="p-inputtext-sm w-40" disabled={isLocked} />
                                        <Button icon="pi pi-plus" label="Crear" size="small" onClick={addSegmento} disabled={isLocked} />
                                        <Button icon="pi pi-filter" label="Else" size="small" severity="warning" outlined onClick={addSegmentoElse} disabled={isLocked} />
                                    </div>
                                ...
                                <Accordion activeIndex={activeSegmentIndex} onTabChange={(e) => setActiveSegmentIndex(e.index)}>
                                    {segmentation.map((seg, idx) => (
                                        <AccordionTab key={seg.id} header={<div className="flex align-items-center justify-content-between w-full"><div className="flex items-center gap-2"><span className="font-bold text-blue-800">{seg.sufijo}</span><Badge value={seg.condiciones?.length > 0 ? `${seg.condiciones.length} Filtros` : (seg.condicion === 'else' ? 'ELSE' : 'Sin Filtros')} severity={idx === 0 ? "info" : "warning"}></Badge></div>{idx > 0 && <Button icon="pi pi-trash" rounded text severity="danger" className="ml-auto p-1" onClick={(e) => { e.stopPropagation(); removeSegment(idx); }} disabled={isLocked} />}</div>}>
                                            <div className="flex flex-col gap-4">
                                                {seg.condicion !== 'else' && (
                                                    <div className="bg-yellow-50 p-3 rounded border border-yellow-200">
                                                        ...
                                                        <div className="grid grid-cols-12 gap-2 items-end mb-2">
                                                            <div className="col-span-4"><Dropdown value={tempSegCol} options={availableColumns} onChange={(e) => setTempSegCol(e.value)} placeholder="Columna" className="w-full p-inputtext-sm" filter disabled={isLocked} /></div>
                                                            <div className="col-span-3"><Dropdown value={tempSegOp} options={operadoresOptions} onChange={(e) => setTempSegOp(e.value)} className="w-full p-inputtext-sm" disabled={isLocked} /></div>
                                                            <div className="col-span-4"><InputText value={tempSegVal} onChange={(e) => setTempSegVal(e.target.value)} placeholder="Valor" className="w-full p-inputtext-sm" disabled={isOpNulo(tempSegOp) || isLocked} /></div>
                                                            <div className="col-span-1"><Button icon="pi pi-plus" size="small" onClick={addConditionToSegment} className="w-full" disabled={isLocked} /></div>
                                                        </div>
                                                        <ul className="text-xs list-disc pl-4">{seg.condiciones?.map((c, i) => (<li key={i} className="flex justify-between items-center mb-1"><span className="font-mono font-bold text-gray-700">{c.columna} <span className="text-blue-600">{c.operador}</span> {c.valor}</span><i className="pi pi-times text-red-500 cursor-pointer" onClick={()=>!isLocked && removeConditionFromSegment(i)}></i></li>))}</ul>
                                                    </div>
                                                )}
                                                <div className="border p-3 rounded">
                                                    ...
                                                    <div className="grid grid-cols-12 gap-2 mb-2 items-end">
                                                        <div className="col-span-5"><InputText placeholder="Columna" value={tempStaticCol} onChange={(e)=>setTempStaticCol(e.target.value)} className="w-full p-inputtext-sm" disabled={isLocked} /></div>
                                                        <div className="col-span-6"><InputText placeholder="Valor" value={tempStaticVal} onChange={(e)=>setTempStaticVal(e.target.value)} className="w-full p-inputtext-sm" disabled={isLocked} /></div>
                                                        <div className="col-span-1"><Button icon="pi pi-plus" size="small" text className="w-full" onClick={addStaticColToSegment} disabled={isLocked} /></div>
                                                    </div>
                                                    <ul className="text-xs pl-4 list-disc">{seg.columnas_estaticas?.map((c, i) => (<li key={i} className="flex justify-between"><span>{c.columna}={c.valor}</span><i className="pi pi-times text-red-500 cursor-pointer" onClick={()=>!isLocked && removeStaticColFromSegment(i)}></i></li>))}</ul>
                                                </div>
                                                <div className="border p-3 rounded bg-gray-50">
                                                    ...
                                                    <div className="grid grid-cols-12 gap-2 mb-3 items-end">
                                                        <div className="col-span-6"><label className="text-xs text-gray-500">Nueva Columna</label><InputText value={tempTargetCol} onChange={(e) => setTempTargetCol(e.target.value)} placeholder="Ej: total" className="w-full p-inputtext-sm" disabled={isLocked} /></div>
                                                        <div className="col-span-6"><label className="text-xs text-gray-500">Tipo</label><Dropdown value={tempTipoDato} options={tiposDatoOptions} onChange={(e) => setTempTipoDato(e.value)} className="w-full p-inputtext-sm" disabled={isLocked} /></div>
                                                    </div>
                                                    <div className="grid grid-cols-12 gap-2 mb-2 items-center bg-white p-2 border rounded">
                                                        ...
                                                        <div className="col-span-4"><Dropdown value={tempCondCol} options={availableColumns} onChange={(e)=>setTempCondCol(e.value)} placeholder="Columna" className="w-full p-inputtext-sm" filter disabled={isLocked} /></div>
                                                        <div className="col-span-3"><Dropdown value={tempCondOp} options={operadoresOptions} onChange={(e)=>setTempCondOp(e.value)} placeholder="Op" className="w-full p-inputtext-sm" disabled={isLocked} /></div>
                                                        <div className="col-span-4"><InputText value={tempCondVal} onChange={(e)=>setTempCondVal(e.target.value)} placeholder="Valor" className="w-full p-inputtext-sm" disabled={isOpNulo(tempCondOp) || isLocked} /></div>
                                                    </div>
                                                    <div className="bg-white p-2 border rounded mb-4">
                                                        <div className="flex gap-2 mb-2 items-center">
                                                            <div className="p-inputgroup flex-1"><InputText value={tempFormula} readOnly className="w-full font-mono bg-gray-100 p-inputtext-sm border-none" placeholder="Usa botones ->" disabled={isLocked} /><Button icon="pi pi-times" className="p-button-danger" onClick={clearFormula} tooltip="Limpiar" disabled={isLocked} /></div>
                                                            <Button icon="pi pi-check" className="p-button-success" onClick={addFormulaToSegment} tooltip="Guardar Cálculo" disabled={isLocked} />
                                                        </div>
                                                        <div className="flex flex-wrap gap-1 justify-between items-center">
                                                            <div className="flex gap-1">
                                                                <Dropdown value={colToInsert} options={availableColumns} onChange={(e)=>{appendToFormula(e.value); setColToInsert(null);}} placeholder="Insertar Col..." className="w-40 p-inputtext-sm" filter disabled={isLocked} />
                                                                {['+', '-', '*', '/', '(', ')'].map(op => <Button key={op} label={op} size="small" outlined className="p-1 w-8 text-center" onClick={() => appendToFormula(op)} disabled={isLocked} />)}
                                                            </div>
                                                            <div className="flex items-center border rounded px-1"><InputNumber value={tempConstante} onValueChange={(e)=>setTempConstante(e.value)} placeholder="#" inputClassName="w-16 p-1 text-sm text-center border-none" disabled={isLocked} /><Button icon="pi pi-plus" text size="small" onClick={()=>{if(tempConstante!==null)appendToFormula(tempConstante.toString());setTempConstante(null)}} disabled={isLocked} /></div>
                                                        </div>
                                                    </div>
                                                    <div className="mt-4 border rounded overflow-hidden">
                                                        <table className="w-full text-sm text-left bg-white">
                                                            ...
                                                            <tbody>{seg.formulas?.map((f, i) => (<tr key={i} className="border-b last:border-0 hover:bg-gray-50"><td className="p-2 font-bold text-blue-600">{f.columna}</td><td className="p-2 font-mono break-all">{f.formula}</td><td className="p-2 text-xs text-gray-500">{f.cond_col ? `${f.cond_col} ${f.cond_op} ${f.cond_val}` : '-'}</td><td className="p-2 text-center"><Button icon="pi pi-trash" rounded text severity="danger" size="small" onClick={() => removeFormulaFromSegment(i)} disabled={isLocked} /></td></tr>))}</tbody>
                                                        </table>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div className="p-5 border border-gray-200 rounded-lg bg-white shadow-sm">
                                ...
                                <div className="flex gap-6"><div className="flex align-items-center"><input type="radio" id="out1" name="salida" value="archivo" checked={modoSalida === 'archivo'} onChange={(e) => setModoSalida(e.target.value)} className="w-4 h-4" disabled={isLocked} /><label htmlFor="out1" className="ml-2">Generar Archivos Excel</label></div><div className="flex align-items-center"><input type="radio" id="out2" name="salida" value="api" checked={modoSalida === 'api'} onChange={(e) => setModoSalida(e.target.value)} className="w-4 h-4" disabled={isLocked} /><label htmlFor="out2" className="ml-2">Enviar a API</label></div></div>
                            </div>
                            <div className="flex justify-end mt-6 gap-3 pt-4 border-t"><Button label="Cancelar" onClick={() => { resetForm(); setActiveIndex(0); }} className={btnSecondary} disabled={isLocked} /><Button label={isEditing ? "Actualizar" : "Guardar"} icon="pi pi-save" onClick={handleGuardar} className={btnPrimary} disabled={isLocked} /></div>
                        </div>
                    </TabPanel>
                )}
            </TabView>
