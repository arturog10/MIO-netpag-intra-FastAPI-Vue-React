...
function GeneradorCampanasPage() {
    ...
    const availableColumns = useMemo(() => {
        ...
        if (currentSegment) {
            if (currentSegment.formulas) {
                currentSegment.formulas.forEach(f => {
                    if (f.columna && !base.find(b => b.value === f.columna.toLowerCase())) {
                        base.push({ label: `${f.columna} (Calc)`, value: f.columna.toLowerCase() });
                    }
                });
            }
            if (currentSegment.columnas_estaticas) {
                currentSegment.columnas_estaticas.forEach(c => {
                    if (c.columna && !base.find(b => b.value === c.columna.toLowerCase())) {
                        base.push({ label: `${c.columna} (Fija)`, value: c.columna.toLowerCase() });
                    }
                });
            }
        }
    ...
    useEffect(() => { if (!selectedEstrategia || !token) { setColumnasDisponibles([]); return; } const fetch = async () => { try { const res = await axios.get(`${API_VISOR_URL}/strategies/load/${selectedEstrategia}`, { headers: { Authorization: `Bearer ${token}` } }); const cols = JSON.parse(res.data.columnas_visibles || "[]"); setColumnasDisponibles(cols.map(c => ({ label: c.header, value: c.field }))); } catch (e) {} }; fetch(); }, [selectedEstrategia, token]);
    ...
    // Segmentos
    const addSegmento = () => {
        if (!tempSegSufijo) { toast.current.show({ severity: 'warn', summary: 'Falta Sufijo', detail: 'Indique el nombre del archivo.' }); return; }
        const newSeg = { id: Date.now(), sufijo: tempSegSufijo, condiciones: [], formulas: [], columnas_estaticas: [] };
        const newSegList = [...segmentation, newSeg];
        setSegmentation(newSegList);
        setTempSegSufijo("");
        setActiveSegmentIndex(newSegList.length - 1);
    };
    ...
    const addConditionToSegment = () => {
        ...
        if (!newSegs[activeSegmentIndex].condiciones) newSegs[activeSegmentIndex].condiciones = [];
        newSegs[activeSegmentIndex].condiciones.push({ columna: tempSegCol, operador: tempSegOp, valor: isOpNulo(tempSegOp) ? '' : tempSegVal });
        setSegmentation(newSegs); setTempSegVal(""); 
    };
    ...
    const addStaticColToSegment = () => {
        ...
        const newSegs = [...segmentation]; 
        if (!newSegs[activeSegmentIndex].columnas_estaticas) newSegs[activeSegmentIndex].columnas_estaticas = [];
        newSegs[activeSegmentIndex].columnas_estaticas.push({ columna: tempStaticCol, valor: tempStaticVal });
        setSegmentation(newSegs); setTempStaticCol(""); setTempStaticVal("");
    };
    ...
    const addFormulaToSegment = () => {
        if (!tempTargetCol || !tempFormula) { toast.current.show({ severity: 'warn', summary: 'Falta info', detail: 'Defina columna y fórmula.' }); return; }
        const newSegs = [...segmentation];
        if (!newSegs[activeSegmentIndex].formulas) newSegs[activeSegmentIndex].formulas = [];
        newSegs[activeSegmentIndex].formulas.push({ columna: tempTargetCol, formula: tempFormula.trim(), cond_col: tempCondCol, cond_op: tempCondOp, cond_val: tempCondVal, tipo: tempTipoDato, rellenar_nulos: 0 });
        setSegmentation(newSegs);
        setTempTargetCol(""); setTempFormula(""); setTempConstante(null); setTempCondCol(null); setTempCondOp(null); setTempCondVal("");
    };
    ...
    const handleEditPlantilla = async (rowData) => {
        ...
        try {
            ...
            try {
                ...
                if (resE.data && resE.data.codigo_cliente) clienteCode = resE.data.codigo_cliente;
            } catch (err) {}
            ...
            if (clienteCode) {
                 ...
                 try {
                    const resList = await axios.get(`${API_VISOR_URL}/strategies/${clienteCode}`, { headers: { Authorization: `Bearer ${token}` } });
                    setEstrategiasDisponibles(resList.data.map(e => ({ label: e.nombre, value: e.id })));
                 } catch(e) {}
            }
            ...
            try {
                ...
                if (proc.segmentacion && Array.isArray(proc.segmentacion)) {
                    const segsSanitized = proc.segmentacion.map(s => ({
                        ...
                        condiciones: Array.isArray(s.condiciones) ? s.condiciones : (s.columna ? [{columna: s.columna, operador: s.operador, valor: s.valor}] : []),
                        formulas: Array.isArray(s.formulas) ? s.formulas : [],
                        columnas_estaticas: Array.isArray(s.columnas_estaticas) ? s.columnas_estaticas : []
                    }));
                    setSegmentation(segsSanitized);
                } else {
                    setSegmentation([{ id: 'base', sufijo: 'BASE (Todos)', condicion: '', formulas: [], columnas_estaticas: [] }]);
                }
            } catch (e) {}
            setModoSalida(plantilla.modo_salida);
            setActiveIndex(1);
            setActiveSegmentIndex(0);
        } catch (error) { toast.current.show({ severity: 'error', summary: 'Error', detail: 'Error carga.' }); } 
    ...
    const fetchResultados = async (taskId) => {
        try {
            ...
            if (res.data.resultados.archivos) { setExecutionResults(res.data.resultados.archivos); setExecutionStats(res.data.resultados.resumen); }
        } catch (e) {}
    };
    const stopPolling = () => { if (pollerRef.current) { clearInterval(pollerRef.current); pollerRef.current = null; } };
    const handleCancel = async () => {
        if (!executingTaskId) return; stopPolling(); setExecutionStatus("cancelled");
        try { await axios.post(`${API_CAMPANAS_URL}/cancel/${executingTaskId}`, {}, { headers: { Authorization: `Bearer ${token}` } }); } catch (e) {}
    };
    
    const handlePreviewFile = async (filePath) => {
        setPreviewTitle(filePath.split('/').pop()); setPreviewVisible(true); setPreviewLoading(true); setPreviewData([]);
        try {
            const encodedPath = encodeURIComponent(filePath);
            const res = await axios.get(`${API_CAMPANAS_URL}/preview?file_path=${encodedPath}`, { headers: { Authorization: `Bearer ${token}` } });
            setPreviewData(res.data);
        } catch (error) { toast.current.show({ severity: 'error', summary: 'Error', detail: 'No se pudo leer.' }); setPreviewVisible(false); } 
        finally { setPreviewLoading(false); }
    };

    const handleDownloadZip = async (filesList) => {
        if (!filesList || filesList.length === 0) return;
        toast.current.show({ severity: 'info', summary: 'Generando ZIP...', detail: 'La descarga comenzará en breve.' });
        try {
            const res = await axios.post(`${API_CAMPANAS_URL}/download-zip`, { files: filesList }, { headers: { Authorization: `Bearer ${token}` }, responseType: 'blob' });
            const url = window.URL.createObjectURL(new Blob([res.data]));
            const link = document.createElement('a'); link.href = url; link.setAttribute('download', `campana_pack_${new Date().getTime()}.zip`); document.body.appendChild(link); link.click(); link.remove();
        } catch (e) { toast.current.show({ severity: 'error', summary: 'Error', detail: 'Error descarga ZIP.' }); }
    };

    const handleDownloadFile = async (e, filePath) => {
        e.stopPropagation();
        try {
            const res = await axios.get(`${API_CAMPANAS_URL}/download-file?file_path=${encodeURIComponent(filePath)}`, { headers: { Authorization: `Bearer ${token}` }, responseType: 'blob' });
            const url = window.URL.createObjectURL(new Blob([res.data]));
            const link = document.createElement('a'); link.href = url; link.setAttribute('download', filePath.split('/').pop()); document.body.appendChild(link); link.click(); link.remove();
        } catch (e) { toast.current.show({ severity: 'error', summary: 'Error', detail: 'Error descarga.' }); }
    };
    ...
    return (
        <div className="w-full card">
            ...
            {/* DIALOGO EXISTENTES */}
            <Dialog header="Archivos Existentes" visible={existingFilesDialogVisible} style={{ width: '50vw' }} onHide={() => setExistingFilesDialogVisible(false)} footer={
                <div className="flex justify-between w-full">
                    <Button label="Descargar Todo (ZIP)" icon="pi pi-download" severity="success" onClick={() => handleDownloadZip(existingFilesList)} />
                    <div className="flex gap-2">
                        <Button label="Cancelar" icon="pi pi-times" onClick={() => setExistingFilesDialogVisible(false)} className="p-button-text" />
                        <Button label="Ejecutar Igual" icon="pi pi-check" onClick={() => runCampaign(pendingRunRowData)} autoFocus />
                    </div>
                </div>
            }>
                <div className="mb-4 text-yellow-600 font-bold"><i className="pi pi-exclamation-triangle mr-2"></i>Ya existen archivos hoy.</div>
                <ul className="bg-gray-50 rounded border p-2 max-h-60 overflow-y-auto divide-y">
                    {existingFilesList.map((f, i) => (
                        <li key={i} className="p-2 hover:bg-blue-100 cursor-pointer flex items-center justify-between text-sm text-blue-700" onClick={() => handlePreviewFile(f)}>
                            <span className="underline truncate flex-1">{f.split('/').pop()}</span>
                            <div className="flex gap-2"><Button icon="pi pi-eye" rounded text size="small" tooltip="Ver" /><Button icon="pi pi-download" rounded text severity="success" size="small" tooltip="Descargar" onClick={(e) => handleDownloadFile(e, f)} /></div>
                        </li>
                    ))}
                </ul>
            </Dialog>
            ...
            {(executionStatus) && (
                <div className={`mb-6 p-4 border rounded-lg shadow-sm ${executionStatus === 'running' ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                    ...
                    {executionStatus === 'running' && <div className="flex items-center gap-3"><ProgressSpinner style={{width:'30px'}}/><span>Procesando...</span><Button label="Cancelar" className={btnSecondary} onClick={handleCancel}/></div>}
                    {executionStatus === 'complete' && (
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <div className="text-green-600 font-bold text-xl">¡Finalizado!</div>
                                <Button label="Descargar Todo (ZIP)" icon="pi pi-download" severity="success" onClick={() => handleDownloadZip(executionResults)} />
                            </div>
                            {executionStats && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    ...
                                    <div className="p-3 bg-red-50 border border-red-100 rounded max-h-40 overflow-y-auto">
                                        ...
                                        <ul className="text-xs list-disc pl-4 text-red-700">
                                            {executionStats.detalle_rechazo && Object.entries(executionStats.detalle_rechazo).map(([motivo, cant]) => (
                                                <li key={motivo}><b>{motivo}:</b> {cant.toLocaleString()}</li>
                                            ))}
                                        </ul>
                                    </div>
                                </div>
                             ...
                             <ul className="bg-white rounded border divide-y">
                                {executionResults.map((file, idx) => (
                                    <li key={idx} className="p-3 hover:bg-blue-50 cursor-pointer flex items-center gap-3 transition-colors group" onClick={() => handlePreviewFile(file)}>
                                        <i className={`pi ${file.includes('RECHAZADOS') ? 'pi-times-circle text-red-500' : 'pi-file-excel text-green-500'} text-xl`}></i>
                                        <span className="text-sm font-mono flex-1 text-gray-700 group-hover:text-blue-600 group-hover:underline break-all">{file.split('/').pop()}</span>
                                        <div className="flex gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                                            <Button icon="pi pi-eye" rounded text severity="info" tooltip="Ver" />
                                            <Button icon="pi pi-download" rounded text severity="success" tooltip="Descargar" onClick={(e) => handleDownloadFile(e, file)} />
                                        </div>
                                    </li>
                                ))}
                            </ul>
                    ...
                    {executionStatus === 'error' && <div className="text-red-600">Error en la ejecución.</div>}
                    {executionStatus === 'cancelled' && <div className="text-orange-600">Cancelado.</div>}
                </div>
            )}
                            ...
                            <div className="p-5 border border-gray-200 rounded-lg bg-white shadow-sm">
                                ...
                                <Accordion activeIndex={activeSegmentIndex} onTabChange={(e) => setActiveSegmentIndex(e.index)}>
                                    {segmentation.map((seg, idx) => (
                                        <AccordionTab key={seg.id} header={
                                            <div className="flex align-items-center justify-content-between w-full">
                                                <div className="flex items-center gap-2">
                                                    <span className="font-bold text-blue-800">{seg.sufijo}</span>
                                                    <Badge value={seg.condiciones?.length > 0 ? `${seg.condiciones.length} Filtros` : (seg.condicion === 'else' ? 'ELSE' : 'Sin Filtros')} severity={idx === 0 ? "info" : "warning"}></Badge>
                                                </div>
                                                {idx > 0 && <Button icon="pi pi-trash" rounded text severity="danger" className="ml-auto p-1" onClick={(e) => { e.stopPropagation(); removeSegment(idx); }} />}
                                            </div>
                                        }>
                                            <div className="flex flex-col gap-4">
                                                ...
                                                {seg.condicion !== 'else' && (
                                                    <div className="bg-yellow-50 p-3 rounded border border-yellow-200">
                                                        ...
                                                        <ul className="text-xs list-disc pl-4">
                                                            {seg.condiciones?.map((c, i) => (
                                                                <li key={i} className="flex justify-between items-center mb-1">
                                                                    <span className="font-mono font-bold text-gray-700">{c.columna} <span className="text-blue-600">{c.operador}</span> {c.valor}</span>
                                                                    <i className="pi pi-times text-red-500 cursor-pointer" onClick={()=>removeConditionFromSegment(i)}></i>
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    </div>
                                                ...
                                                <div className="border p-3 rounded">
                                                    ...
                                                    <ul className="text-xs pl-4 list-disc">
                                                        {seg.columnas_estaticas?.map((c, i) => (<li key={i} className="flex justify-between"><span>{c.columna}={c.valor}</span><i className="pi pi-times text-red-500 cursor-pointer" onClick={()=>removeStaticColFromSegment(i)}></i></li>))}
                                                    </ul>
                                                </div>
                                                    ...
                                                    <div className="mt-4 border rounded overflow-hidden">
                                                        <table className="w-full text-sm text-left bg-white">
                                                            <thead className="bg-gray-100 text-gray-600 border-b">
                                                                <tr><th className="p-2">Columna</th><th className="p-2">Fórmula</th><th className="p-2">Condición</th><th className="p-2 w-10"></th></tr>
                                                            </thead>
                                                            <tbody>
                                                                {seg.formulas?.map((f, i) => (
                                                                    <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                                                                        <td className="p-2 font-bold text-blue-600">{f.columna}</td>
                                                                        <td className="p-2 font-mono break-all">{f.formula}</td>
                                                                        <td className="p-2 text-xs text-gray-500">{f.cond_col ? `${f.cond_col} ${f.cond_op} ${f.cond_val}` : '-'}</td>
                                                                        <td className="p-2 text-center"><Button icon="pi pi-trash" rounded text severity="danger" size="small" onClick={() => removeFormulaFromSegment(i)} /></td>
                                                                    </tr>
                                                                ))}
                                                            </tbody>
                                                        </table>
